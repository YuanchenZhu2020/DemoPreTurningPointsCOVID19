...
#' to make removed_rate less than 1.
#'
...
#' new_velocity <- corr_removed_rate(velocity, indicators, pre_data, M, Begining_Time)
corr_removed_rate <- function(velocity, indicators, pre_data, M, Begining_Time) {
  ...
  # This while loop is used to avoid removed_rate > 1, which is counterintuitive.
  while (removed_rate.pre[length(removed_rate.pre)] > 1 & T_I_ind >= 1) {
    ...
    pre_data_tmp <- DemoPreTurningPointsCOVID19::get_future_indicators(indicators, velocity, Begining_Time)
